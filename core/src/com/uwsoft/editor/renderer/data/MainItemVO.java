...
public class MainItemVO {
	...
	public void loadFromEntity(Entity entity) {
		MainItemComponent mainItemComponent = entity.getComponent(MainItemComponent.class).get();
		TransformComponent transformComponent = entity.getComponent(TransformComponent.class).get();
		TintComponent tintComponent = entity.getComponent(TintComponent.class).get();
		ZIndexComponent zindexComponent = entity.getComponent(ZIndexComponent.class).get();
		 ...
		 */
		PolygonComponent polygonComponent = entity.getComponent(PolygonComponent.class).get();
		if(polygonComponent != null && polygonComponent.vertices != null) {
			shape = new ShapeVO();
			shape.polygons = polygonComponent.vertices;
		}
        PhysicsBodyComponent physicsComponent = entity.getComponent(PhysicsBodyComponent.class).get();
        if(physicsComponent != null) {
            physics = new PhysicsBodyDataVO();
            physics.loadFromComponent(physicsComponent);
        }

		ShaderComponent shaderComponent = entity.getComponent(ShaderComponent.class).get();
